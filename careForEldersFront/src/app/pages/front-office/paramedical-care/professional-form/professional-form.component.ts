...
export class ProfessionalFormComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode && this.professionalId) {
      ...
      this.paramedicalService.updateProfessional(this.professionalId, updateData)
        .pipe(finalize(() => { this.submitting = false; }))
        .subscribe({
          next: () => {
            this.router.navigate(["/user/userProfile/paramedical-care/professional-list"])
          },
          error: (err) => {
      ...
      this.paramedicalService.createProfessional(formData)
        .pipe(finalize(() => { this.submitting = false; }))
        .subscribe({
          next: (response) => {
            this.router.navigate(["/user/userProfile/paramedical-care/professional-list"])
  ...
  cancel(): void {
    if (this.isEditMode && this.professionalId) {
      this.router.navigate(["/user/userProfile/paramedical-care/professional-detail", this.professionalId])
    } else {
      this.router.navigate(["/user/userProfile/paramedical-care/professional-list"])
    }
  }
