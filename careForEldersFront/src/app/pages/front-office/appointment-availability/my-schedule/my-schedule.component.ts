...
navigateToday(): void {
 ...
 eventClicked({ event }: { event: CalendarEvent }): void {
  const isBooked =
    event.meta?.type === 'availability' &&
    (
      event.meta?.isBooked === true ||                // already marked as booked
      !!event.meta?.data?.reservationId               // or a reservation exists for this slot
    );
  ...
  this.dialog.open(EventDetailsDialogComponent, {
    ...
    data: {
      ...
      meta: {
        ...event.meta,
        isBooked // always include this property!
      }
    }
  });
